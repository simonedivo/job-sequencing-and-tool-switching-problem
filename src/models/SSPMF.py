...
class SSPMFModel:
    ...
    def setup_constraints(self):
        ...
        # len(self.job_nodes)-2 equal N-2 while len(self.job_nodes)-1 equals N-1
        for t in self.tools:
        ...
        # range() of len(self.job_nodes)-1 equals N-1
        for t in self.tools:
            self.model.addConstr(gp.quicksum(self.y[i,self.N2,t] for i in range(1, len(self.job_nodes)-1)) - gp.quicksum(self.y[self.N2,i,t] for i in range(1,len(self.job_nodes))) == 0, name=f"FlowConservation(2i)")

        for i in self.job_nodes:
            for k in self.job_nodes:
                if i in self.job_tools_requirements:
                    for t in self.job_tools_requirements[i]:
        ...
        p = max(self.jobs, key=lambda j: len(self.job_tools_requirements[j]))
        self.model.addConstr(gp.quicksum(self.x[p,k] for k in range(1, math.ceil(len(self.job_nodes)/2)+1)) == 1, name="JobMaxToolsFirstHalf(2l)")

        for t in self.tools:
            num_jobs_need_t = sum(1 for job in self.jobs if t in self.job_tools_requirements[job])
            for k in range(1, num_jobs_need_t + 1):
    ...
    def setup_objective(self):
        ...
        # again, with range() function the length is 1 unit shorter -> len(self.job_nodes) is N-1 and len(self.job_nodes)-1 is N-2
    ...
    def get_solution(self):
        ...
        if self.model.status == gp.GRB.OPTIMAL:
            job_order = sorted((k, i) for i in self.jobs for k in self.num_nodes if self.x[i, k].x > 0.5)
